...
public abstract class DAO_BD extends DAO {
	...
	public static Connection getConexao(String pNmConexaoJNDI, boolean pInLookupGlobal)
		...
		try {
			...
			throw new ExcecaoSistema("Conex�o JNDI n�o dispon�vel (pNmConexaoJNDI = " + pNmConexaoJNDI + " | NmProviderURL = " +
	...
	public static void fecharConexao(ContextoUsuario pContextoUsuario, String pNmClasse, Connection pConexao,
		...
		// Se for transa��o JTA_CRGC ou JDBC_RGC, n�o fecha nada. Quem deve fechar a conex�o e o resultset � o cliente.
		if (
				(
			...
			try {
				...
				if (pContextoUsuario.getTpTransacao() == Constantes.TP_TRANSACAO_JTA) {
					// Se for transa��o JTA, fecha a conex�o devolvendo para o pool
					if (pConexao != null) {
						pConexao.close();
					}
				}
			} catch (SQLException e) {
				DAO.tratarExcecao(pNmClasse, "pContextoUsuario = " + pContextoUsuario + " | Erro fechando conex�o", e);
			}
		}
	...
	/**
	 * Transforma o resultset passado como par�metro em uma cole��o de VOGenerico
	 *
	 * @param pResultSet
	 *
	 * @return
	...
	/**
	 * Transforma o resultset passado como par�metro em um VOGenerico
	 *
	 * @param pResultSet
	 *
	 * @return
	...
	public static void configurarConexaoTransacaoJDBC(Connection pConexao)
		...
		try {
			...
			String nmUsuarioBD = "[n�o dispon�vel]";
			...
			throw new ExcecaoSistema("Erro configurando conex�o (Usu�rio BD: " + nmUsuarioBD + ") | sql = " + sql, e);
		}
	}
	...
	public void incluirBatch(ContextoUsuario pContextoUsuario, ArrayList<?extends VOEntidade> pArrayVOEntidade)
		...
		for (int i = 0; i < pArrayVOEntidade.size(); i++) {
			...
			if (i == 0) {
				// Verifica se a inclus�o em batch est� ativada ou desativada para o sistema em quest�o
		...
		if (!isUtilizacaoBatchDesativado) {
			try {
				...
				for (VOEntidade voEntidade : pArrayVOEntidade) {
					...
					// Adiciona os valores setados no statement para execu��o batch
					preStmt.addBatch();
		...
		try {
			...
			if (preStmt.getUpdateCount() == 0) {
				...
				throw new ExcecaoRegistroNaoEncontrado(pVOEntidade.getDsEntidade(),
					this.getClass().getName() + " | m�todo = alterar | pContextoUsuario = " + pContextoUsuario +
	...
	public void alterarBatch(ContextoUsuario pContextoUsuario, ArrayList<?extends VOEntidade> pArrayVOEntidade)
		...
		for (int i = 0; i < pArrayVOEntidade.size(); i++) {
			...
			if (i == 0) {
				// Verifica se a utiliza��o de update em batch est� ativada ou desativada para o sistema em quest�o
				isUtilizacaoBatchDesativado = Parametros.getInstancia()
						.isUtilizacaoBatchDesativado(BibliotecaFuncoesPrincipal.extrairSgSistema(voEntidade.getClass().getName())
							.toUpperCase());
		...
		if (!isUtilizacaoBatchDesativado) {
			try {
				// Armazena o grupo de campos que ser�o atualizados, cada um com um update em batch.
				...
				for (VOEntidade voEntidade : pArrayVOEntidade) {
					...
					// Se este grupo j� foi adicionado por algum voEntidade anterior...
					if (grupoCamposUpdate.containsKey(camposAlterados.toString())) {
						grupoCamposUpdate.get(camposAlterados.toString()).add(voEntidade);
					} else {
						// Como este grupo ainda n�o adicionado, adiciona no hashtable
				...
				while (keysCamposUpdate.hasMoreElements()) {
					...
					// Se algum dos updates executados em batch n�o retornou nenhum resultado a ExcecaoRegistroNaoEncontrado � levantada. 
					for (int i = 0; i < updateCounts.length; i++) {
						...
						if (updateCount == 0) {
							...
							throw new ExcecaoRegistroNaoEncontrado(dsEntidade,
								this.getClass().getName() + " | m�todo = alterarBatch | pContextoUsuario = " + pContextoUsuario +
	...
	public void excluir(ContextoUsuario pContextoUsuario, VOEntidade pVOEntidade)
		...
		try {
			...
			if (preStmt.getUpdateCount() == 0) {
				...
				throw new ExcecaoRegistroNaoEncontrado(pVOEntidade.getDsEntidade(),
					this.getClass().getName() + " | m�todo = excluir | pContextoUsuario = " + pContextoUsuario +
					" | pVOEntidade = " + pVOEntidade);
			}
		} finally {
			this.fecharConexao(pContextoUsuario, conexao, preStmt);
	...
	public VOGenerico consultarPorChavePrimaria(ContextoUsuario pContextoUsuario, VOEntidade pVOEntidade, boolean pInExtrairBLOB,
		...
		try {
			...
			if (!rs.next()) {
				...
				throw new ExcecaoRegistroNaoEncontrado(pVOEntidade.getDsEntidade(),
					this.getClass().getName() + " | m�todo = consultarPorChavePrimaria | pContextoUsuario = " + pContextoUsuario +
					" | pVOEntidade = " + pVOEntidade);
			}
	...
	/**
	 * Este m�todo n�o trata o tipo CLOB. Ele � tratado nos m�todos getResultSetComoVOGenerico e getResultSetComoColecao
	 *
	 * @param pTpColuna
	 *
	 * @return
	...
	protected static String getDsNivelIsolamento(int pCdNivelIsolamento) {
		...
		if (pCdNivelIsolamento == Connection.TRANSACTION_NONE) {
			...
			nmNivelIsolamento = "N�o indentificado";
		}
