...
public class Parametros {
	...
	private final List<String> aNmClassesUtilitariasPadrao = new ArrayList<>();
...
/**
		...
		if (daoParametro == null) {
			daoParametro = new DAO_TXT() {
						...
						protected void sinalizarCargaPropriedades(Properties pPropriedades) {
							...
							// Redefine o intervalo (em segundos) de atualização de propriedades padrão (de todos os DAO_TXT).
							try {
							...
							// Redefine o intervalo (em segundos) de atualização de propriedades dos DAO_TXT utilizados nesta classe.
							try {
	...
	public String getParametroPropriedade(String pSgArea, String pSgSistema, String pNmParametro)
		...
		try {
			...
			if (e instanceof ExcecaoGenerica) {
				...
				throw new ExcecaoSistema("Erro consultando parâmetro (pSgSistema = " + pSgSistema + " | pNmParametro = " +
					pNmParametro + ")", e);
			}
		}
	...
	private <T> T consultarDAOComCache(DAO_TXT pDAO, Map<String, Object> pCache, String pNmPropriedade,
		...
		if (pDAO != null) {
			try {
				// A atualização já foi verificada no início, não é necessária no "getPropriedade".
		...
		try {
			...
			throw new RuntimeException("Erro convertendo valor do parâmetro [" + pNmPropriedade + "] do arquivo " +
	...
	private <T> T consultarDAOParametro(String pNmPropriedade, ConversorPropriedade<T> pConversor) {
		...
		try {
			...
			// Primeiro tenta atualizar. Pois, caso haja atualização, o cache será substituído.
			dao.atualizarPropriedadesPorTempo();
		} catch (Exception e) {
		}

		// ATENÇÃO: o método "getDAOParametro" deve realizar a limpeza da variável "aCacheParametros" a cada recarga das propriedades.
	...
	private <T> T consultarDAOParametroDinamico(String pNmPropriedade, ConversorPropriedade<T> pConversor) {
		...
		// ATENÇÃO: o método "getDAOParametroDinamico" deve realizar a limpeza da variável "aCacheParametrosDinamicos" a cada recarga
		// das propriedades.
		...
		return this.consultarDAOParametroDinamico("QtLimiteRegistrosConsulta",
			new ConversorPropriedade<Integer>() {
				public Integer converter(String pVlPropriedade)
					...
					// Incrementa caso não seja número par.
					if ((vlParametro % 2) != 0) {
						vlParametro += 1;
					}
	...
	public String getVlHTTPHeaderCacheControl() {
		// Devido a grande quantidade de acessos a este parâmetro, é realizado cache estático (em variável de classe específica) no
		// lugar do cache dinâmico (via "consultarDAOComCache").
		// ATENÇÃO: o método "getDAOParametro" deve realizar a limpeza da variável "aVlHTTPHeaderCacheControl" a cada recarga
		// das propriedades.
	...
	public String getQtRegistrosPorPaginaPorRede(String pRede) {
		// Não realiza cache para este parâmetro porque será consultado para cada IP. Isto acumularia uma quantidade muito grande de
	...
	public boolean isTraceAtivado() {
		// Devido a grande quantidade de acessos a este parâmetro, é realizado cache estático (em variável de classe específica) no
		// lugar do cache dinâmico (via "consultarDAOComCache").
		// ATENÇÃO: o método "getDAOParametroDinamico" deve realizar a limpeza da variável "aVlTraceAtivado" a cada recarga
		// das propriedades.
	...
	public boolean isTraceFachadaAtivado() {
		// Devido a grande quantidade de acessos a este parâmetro, é realizado cache estático (em variável de classe específica) no
		// lugar do cache dinâmico (via "consultarDAOComCache").
		// ATENÇÃO: o método "getDAOParametroDinamico" deve realizar a limpeza da variável "aVlTraceFachadaAtivado" a cada recarga
		// das propriedades.
	...
	public boolean isTraceEJBAtivado() {
		// Devido a grande quantidade de acessos a este parâmetro, é realizado cache estático (em variável de classe específica) no
		// lugar do cache dinâmico (via "consultarDAOComCache").
		// ATENÇÃO: o método "getDAOParametroDinamico" deve realizar a limpeza da variável "aVlTraceEJBAtivado" a cada recarga
		// das propriedades.
	...
	public int getTraceTransacaoAtivado() {
		// Devido a grande quantidade de acessos a este parâmetro, é realizado cache estático (em variável de classe específica) no
		// lugar do cache dinâmico (via "consultarDAOComCache").
		// ATENÇÃO: o método "getDAOParametroDinamico" deve realizar a limpeza da variável "aVlTraceTransacaoAtivado" a cada recarga
		// das propriedades.
	...
	public Set<String> getTraceNmClassesUtilitarias() {
		// Devido a grande quantidade de acessos a este parâmetro, é realizado cache estático (em variável de classe específica) no
		// lugar do cache dinâmico (via "consultarDAOComCache").
		// ATENÇÃO: o método "getDAOParametroDinamico" deve realizar a limpeza da variável "aVlTraceNmClassesUtilitarias" a cada recarga
		// das propriedades.
			...
			if (vlPropriedade != null) {
				...
				for (String nmClasse : nmClasses) {
					...
					if (this.isTraceAtivado()) {
						String txLogParte01 = "Parâmetro 'TraceNmClassesUtilitarias': adicionada classe '";
	...
	public int getTamanhoArquivoLogTraceTransacaoPorUsuario(String pCdUsuario) {
		// Não realiza cache para este parâmetro porque será consultado para cada usuário. Isto acumularia uma quantidade muito grande
		// de propriedades no cache. Além disto, o padrão é estar desabilitado em produção.
		...
		if (strVlTraceUsuario != null) {
			try {
				...
				// Parâmetro não crítico. Pode abafar.
			}
		}
	...
	public boolean isJ2EE14Ativado() {
		// A partir de 23/12/2010, esse párâmetro ficou ficou definitivamente ativado.
		return true;
	}
	...
	/**
	 * Parâmetro que retorna a URL base para referenciar o applet de assintura digital. Ex:
	...
	private abstract static class ConversorPropriedade<T> {
		...
		public static Boolean converterBoolean(String pVlPropriedade)
			...
			if (pVlPropriedade.equals("1")) {
				...
				throw new ExcecaoParametroInvalido("Valor de parâmetro boleano inválido: " + pVlPropriedade);
			}
		}
		...
		/**
		 * Deve ser sobreescrito para tratamento de valores mutáveis. O objetivo é realizar uma cópia do objeto em
		 * cache para que alterações no objeto retornado não afetem o cache.
		 *
		 * @param pVlParametro
		 *
		 * @return
